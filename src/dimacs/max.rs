...
pub fn read<R: Read, G>(reader: R) -> Result<Instance<G>>
where
    G: IndexDigraph + Buildable,
{
    let mut reader = DimacsReader::new(reader);
    ...
    let nodes = builder.add_nodes(nnodes);
    let mut src = None;
    let mut snk = None;
    ...
    for _ in 0..nedges {
        ...
        let e = builder.add_edge(nodes[u - 1], nodes[v - 1]);
        upper[builder.edge2id(e)] = c;
    }
    ...
    let src = src.unwrap() - 1;
    let snk = snk.unwrap() - 1;
    Ok(Instance { graph, src, snk, upper })
}

pub fn read_from_file<G>(filename: &str) -> Result<Instance<G>>
where
    G: IndexDigraph + Buildable,
{
...
pub fn write<W, G>(mut w: W, instance: &Instance<G>) -> std::io::Result<()>
where
    W: Write,
    G: IndexDigraph,
{
    ...
    writeln!(w, "n {} s", instance.src + 1)?;
    writeln!(w, "n {} t", instance.snk + 1)?;
    for e in g.edges() {
        let eid = g.edge_id(e);
        writeln!(
            w,
...
pub fn write_to_file<W, G>(filename: &str, instance: &Instance<G>) -> std::io::Result<()>
where
    W: Write,
    G: IndexDigraph,
{
